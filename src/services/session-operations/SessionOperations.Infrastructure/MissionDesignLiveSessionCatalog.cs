...
public sealed class MissionDesignLiveSessionCatalog(HttpClient httpClient) : IMissionDesignLiveSessionCatalog
{
        ...
        CancellationToken cancellationToken)
    {
        var response = await httpClient.GetAsync(
            $"api/mission-design/missions/eligible-for-live-session/{missionId}",
            cancellationToken);
        ...
        if (mission is null)
        {
            throw new UmbralTechnicalException(
                "mission_design_live_session_catalog_empty_payload",
                $"Mission Design returned an empty payload for Mission '{missionId}'.");
        }
    ...
    private static async Task<UmbralServiceException> CreateUpstreamExceptionAsync(
        ...
        CancellationToken cancellationToken)
    {
        var detail = $"Mission Design request failed for Mission '{missionId}'.";
        string? code = null;
        ...
        try
        {
            ...
            var problemDetails = await JsonSerializer.DeserializeAsync<MissionDesignProblemDetails>(
                contentStream,
                cancellationToken: cancellationToken);
        ...
        var failureCode = string.IsNullOrWhiteSpace(code)
            ? $"mission_design_live_session_catalog_{(int)response.StatusCode}"
            : code;
    ...
    private sealed record MissionDesignProblemDetails(
        string? Detail,
        string? Code,
        string? Category);
}
