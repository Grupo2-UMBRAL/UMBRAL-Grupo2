...
public sealed class SessionManagementHealthEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly WebApplicationFactory<Program> factory;

    public SessionManagementHealthEndpointTests(WebApplicationFactory<Program> factory)
    {
...
public sealed class SessionManagementRoleSmokeRouteTests
{
    [Fact]
    public async Task SmokeRoute_ReturnsUnauthorized_WhenRequestHasNoIdentity()
    {
        ...
        var response = await client.GetAsync("/api/session-management/smoke/operator");
...
public sealed class LiveSessionEndpointTests
{
    ...
    public async Task CreateLiveSession_ReturnsCreatedSessionWithSelectedFlow()
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/api/session-management/live-sessions",
    ...
    public async Task LiveSessionRoutes_ReturnForbidden_ForParticipant()
    {
        ...
        var response = await client.GetAsync("/api/session-management/live-sessions");
    ...
    public async Task LifecycleStart_ReturnsConflict_WhenNoTeamsRegistered()
    {
        ...
        var response = await operatorClient.PostAsync(
            $"/api/session-management/live-sessions/{liveSession.Id}/lifecycle/start",
            content: null);
    ...
    public async Task LifecycleFlow_TransitionsState_ClosesEnrollmentWindow_AndNotifiesRealtime()
    {
        ...
        var windowResponse = await operatorClient.PostAsync(
            $"/api/session-management/live-sessions/{liveSession.Id}/session-enrollment/window/open",
            content: null);
        windowResponse.EnsureSuccessStatusCode();
        var registerResponse = await participantClient.PostAsJsonAsync(
            "/api/session-management/session-enrollment/teams",
        ...
        var pauseResponse = await operatorClient.PostAsync(
            $"/api/session-management/live-sessions/{liveSession.Id}/lifecycle/pause",
            content: null);
        var resumeResponse = await operatorClient.PostAsync(
            $"/api/session-management/live-sessions/{liveSession.Id}/lifecycle/resume",
            content: null);
        var finalizeResponse = await operatorClient.PostAsync(
            $"/api/session-management/live-sessions/{liveSession.Id}/lifecycle/finalize",
            content: null);
    ...
    private static async Task<string> GenerateJoinCodeAsync(HttpClient client, Guid liveSessionId)
    {
        var response = await client.PostAsync(
            $"/api/session-management/live-sessions/{liveSessionId}/session-enrollment/join-code",
            content: null);
        response.EnsureSuccessStatusCode();
...
internal sealed class SessionManagementApiFactory : WebApplicationFactory<Program>
{
    private readonly string databaseName = $"session-management-tests-{Guid.NewGuid():N}";
    ...
    protected override void ConfigureWebHost(Microsoft.AspNetCore.Hosting.IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            services.RemoveAll<DbContextOptions<SessionManagementDbContext>>();
            services.RemoveAll<SessionManagementDbContext>();

            services.AddScoped<SessionManagementDbContext>(sp =>
            {
            ...
            services.AddScoped<DbContextOptions<SessionManagementDbContext>>(sp =>
            {
                return new DbContextOptionsBuilder<SessionManagementDbContext>()
                    .UseInMemoryDatabase(databaseName)
                    .Options;
            });
    ...
    private sealed class FakeLiveSessionStateNotifier : ISessionRealtimeNotifier
    {
