...
public sealed class OpenApiContractTests
{
    ...
    public async Task Api_PublishesOpenApiDocumentWithoutInteractiveReference()
    {
        ...
        Assert.True(paths.TryGetProperty("/api/session-management/live-sessions", out _));
    ...
    public async Task OpenApiDocument_ExcludesRealtimeHub()
    {
        ...
        Assert.False(document.RootElement.GetProperty("paths").TryGetProperty("/hubs/session", out _));
    }

    [Fact]
    public async Task OpenApiDocument_DescribesLiveSessionCollectionOperation()
    {
        await using var factory = new SessionManagementApiFactory();
        var client = factory.CreateClient();
        ...
        var operation = document.RootElement
            .GetProperty("paths")
            .GetProperty("/api/session-management/live-sessions")
            .GetProperty("get");

        Assert.Equal("List live sessions", operation.GetProperty("summary").GetString());
    ...
    public async Task OpenApiDocument_AdvertisesForwardedServerWhenProxied()
    {
        ...
        request.Headers.Add("X-Forwarded-Prefix", "/session-management");

        var response = await client.SendAsync(request);
        using var document = JsonDocument.Parse(await response.Content.ReadAsStreamAsync());
        var server = document.RootElement.GetProperty("servers")[0].GetProperty("url").GetString();

        Assert.Equal("https://umbral.example.org/session-management", server);
        ...
        Assert.True(document.RootElement
            .GetProperty("paths")
            .TryGetProperty("/api/session-management/live-sessions", out _));
    }
    ...
    public async Task OpenApiDocument_DescribesSchemaPropertiesFromXmlComments()
    {
        ...
        var properties = document.RootElement
            .GetProperty("components")
            .GetProperty("schemas")
            .GetProperty("CreateLiveSessionRequest")
            .GetProperty("properties");

        Assert.False(string.IsNullOrWhiteSpace(
            properties.GetProperty("missionId").GetProperty("description").GetString()));
        Assert.False(string.IsNullOrWhiteSpace(
            properties.GetProperty("selectedMissionStageIds").GetProperty("description").GetString()));
    }
}
