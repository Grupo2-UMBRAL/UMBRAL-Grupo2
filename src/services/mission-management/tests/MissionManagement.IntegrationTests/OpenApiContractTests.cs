...
public sealed class OpenApiContractTests
{
    ...
    public async Task Api_PublishesOpenApiDocumentAndInteractiveReference()
    {
        ...
        Assert.True(paths.TryGetProperty("/api/mission-management/missions", out _));
    ...
    [Fact]
    public async Task OpenApiDocument_DescribesMissionCollectionOperation()
    {
        await using var factory = new MissionApiFactory();
        var client = factory.CreateClient();
        ...
        var operation = document.RootElement
            .GetProperty("paths")
            .GetProperty("/api/mission-management/missions")
            .GetProperty("get");

        Assert.Equal("List reusable missions", operation.GetProperty("summary").GetString());
    ...
    public async Task OpenApiDocument_AdvertisesForwardedServerWhenProxied()
    {
        ...
        request.Headers.Add("X-Forwarded-Prefix", "/mission-management");

        var response = await client.SendAsync(request);
        using var document = JsonDocument.Parse(await response.Content.ReadAsStreamAsync());
        var server = document.RootElement.GetProperty("servers")[0].GetProperty("url").GetString();

        Assert.Equal("https://umbral.example.org/mission-management", server);
        ...
        Assert.True(document.RootElement
            .GetProperty("paths")
            .TryGetProperty("/api/mission-management/missions", out _));
    }
    ...
    public async Task OpenApiDocument_DescribesSchemaPropertiesFromXmlComments()
    {
        ...
        var properties = document.RootElement
            .GetProperty("components")
            .GetProperty("schemas")
            .GetProperty("CreateMissionRequest")
            .GetProperty("properties");

        Assert.False(string.IsNullOrWhiteSpace(
            properties.GetProperty("name").GetProperty("description").GetString()));
    }
}
