...
type MissionNodeEditorProps = {
  ...
  onUpdateNode: (nodeClientId: string, field: keyof MissionNodeDraft, value: string | boolean) => void;
};
...
function createEmptyNodeDraft(kind: "leaf" | "composite", order: number): MissionNodeDraft {
  return {
    clientId: createClientId(),
    name: "",
    order: String(order),
...
function toNodeDraft(node: MissionNode): MissionNodeDraft {
  return {
    ...
    triviaInitialValidationCriterion: node.triviaInitialValidationCriterion ?? "",
  ...
  return nodes.reduce<MissionTreeStats>(
    (summary, node) => {
      ...
      return {
        ...
        leafStages: summary.leafStages + (node.children.length === 0 ? 1 : 0) + childSummary.leafStages,
        compositeBlocks:
          summary.compositeBlocks + (node.children.length > 0 ? 1 : 0) + childSummary.compositeBlocks,
...
function getNextOrder(nodes: MissionNodeDraft[]) {
  return nodes.reduce((maximum, node) => {
    ...
    return Number.isFinite(numericOrder) ? Math.max(maximum, numericOrder) : maximum;
  }, 0) + 1;
}
...
function removeNodeFromTree(nodes: MissionNodeDraft[], targetClientId: string): MissionNodeDraft[] {
  return nodes
...
function serializeNodeDraft(node: MissionNodeDraft): MissionNodePayload {
  ...
  return {
    ...
    isActive: node.isActive,
    defaultTimeBudgetMinutes: isLeaf ? null : parseOptionalInteger(node.defaultTimeBudgetMinutes),
    timeBudgetMinutes: isLeaf ? parseOptionalInteger(node.timeBudgetMinutes) : null,
    gameType: isLeaf ? node.gameType : null,
    expectedQrHash: isLeaf && node.gameType === "Treasure Hunt" ? trimToNull(node.expectedQrHash) : null,
    triviaValidAnswer: isLeaf && node.gameType === "Trivia" ? trimToNull(node.triviaValidAnswer) : null,
    triviaInitialValidationCriterion:
      isLeaf && node.gameType === "Trivia" ? trimToNull(node.triviaInitialValidationCriterion) : null,
...
function findValidationIssue(
  ...
  for (const node of nodes) {
    ...
    const issue = findValidationIssue(node.children, nextInheritedBudget, label);
    if (issue) {
      return issue;
    }
  }
...
function MissionNodeEditor({
  ...
  return (
    <article className="mission-node-card" style={{ marginLeft: `${depth * 16}px` }}>
      <div className="mission-node-header">
        <div>
          <div className="node-chip-row">
            <span className={isLeaf ? "node-chip is-leaf" : "node-chip is-composite"}>
              {isLeaf ? "Leaf stage" : "Composite block"}
            </span>
            <span className={node.isActive ? "status-pill status-ok" : "status-pill status-error"}>
              {node.isActive ? "active" : "inactive"}
            </span>
          </div>
          <h4>{node.name.trim() || (isLeaf ? "Untitled stage" : "Untitled block")}</h4>
          <p className="muted-copy">
            {isLeaf
              ? "Playable stage with game validation, hints, and optional local time budget."
              : "Structural block. Children inherit mission time budget unless the block overrides it."}
          </p>
        </div>

        <div className="node-actions">
          {isLeaf ? (
            <button className="ghost-button" onClick={() => onMakeComposite(node.clientId)} type="button">
              Make block
            </button>
          ) : (
            <button className="ghost-button" onClick={() => onMakeLeaf(node.clientId)} type="button">
              Clear to leaf
            </button>
          )}
          <button className="ghost-button danger-button" onClick={() => onRemoveNode(node.clientId)} type="button">
            Remove
          </button>
        </div>
      </div>
      ...
      <div className="node-grid">
        <label className="field">
          ...
          <input
            className="input"
            maxLength={120}
            onChange={(event) => onUpdateNode(node.clientId, "name", event.target.value)}
            required
            value={node.name}
          />
        </label>
        ...
        <label className="field">
          ...
          <input
            className="input"
            min={1}
            onChange={(event) => onUpdateNode(node.clientId, "order", event.target.value)}
            required
            type="number"
            value={node.order}
          />
        ...
        <label className="field inline-toggle">
          ...
          <input
            ...
            onChange={(event) => onUpdateNode(node.clientId, "isActive", event.target.checked)}
            type="checkbox"
          />
        </label>
      </div>
      ...
      {isLeaf ? (
        <>
          <div className="node-grid">
            <label className="field">
              ...
              <input
                className="input"
                min={1}
                onChange={(event) => onUpdateNode(node.clientId, "timeBudgetMinutes", event.target.value)}
                type="number"
                value={node.timeBudgetMinutes}
              />
              <span className="field-hint">Leave empty to inherit the mission or parent block budget.</span>
            </label>
            ...
            <label className="field">
              ...
              <select
                className="input"
                onChange={(event) => onUpdateNode(node.clientId, "gameType", event.target.value)}
                value={node.gameType}
              >
          ...
          {node.gameType === "Treasure Hunt" ? (
            <label className="field">
              <span>Expected QR hash</span>
              <input
                className="input"
                maxLength={256}
                onChange={(event) => onUpdateNode(node.clientId, "expectedQrHash", event.target.value)}
                required
                value={node.expectedQrHash}
              />
            </label>
          ) : null}
            ...
            <div className="node-grid">
              <label className="field">
                ...
                <input
                  className="input"
                  maxLength={512}
                  onChange={(event) => onUpdateNode(node.clientId, "triviaValidAnswer", event.target.value)}
                  value={node.triviaValidAnswer}
                />
              </label>
              ...
              <label className="field">
                ...
                <input
                  ...
                  maxLength={512}
                  onChange={(event) =>
                    onUpdateNode(node.clientId, "triviaInitialValidationCriterion", event.target.value)
                  }
                  value={node.triviaInitialValidationCriterion}
                />
              </label>
            </div>
          ) : null}
          ...
          <section className="node-subsection">
            <div className="mission-list-header">
              ...
              <button className="ghost-button" onClick={() => onAddHint(node.clientId)} type="button">
                Add hint
              </button>
            </div>
            ...
            {node.hints.length === 0 ? (
              <div className="tree-empty-state">
                ...
                <p>Treasure Hunt and Trivia stages can carry visible clues or final solution drops.</p>
              </div>
            ) : null}

            <div className="hint-editor-list">
              {node.hints.map((hint, index) => (
                <article className="hint-card" key={hint.clientId}>
                  <div className="mission-list-header">
                    <div>
                    ...
                    <button
                      ...
                      onClick={() => onRemoveHint(node.clientId, hint.clientId)}
                      type="button"
                    >
                      Remove
                    </button>
                  ...
                  <label className="field">
                    ...
                    <textarea
                      ...
                      maxLength={1024}
                      onChange={(event) => onUpdateHint(node.clientId, hint.clientId, "content", event.target.value)}
                      required
                      rows={3}
                      value={hint.content}
                    />
                  ...
                  <div className="node-grid">
                    <label className="field inline-toggle">
                      <span>Reveal as solution</span>
                      <input
                        checked={hint.isSolution}
                        onChange={(event) =>
                          onUpdateHint(node.clientId, hint.clientId, "isSolution", event.target.checked)
                        }
                        type="checkbox"
                      />
                    </label>
                    ...
                    <label className="field">
                      ...
                      <input
                        className="input"
                        onChange={(event) => onUpdateHint(node.clientId, hint.clientId, "latitude", event.target.value)}
                        type="number"
                        value={hint.latitude}
                      />
                    </label>
                    ...
                    <label className="field">
                      ...
                      <input
                        className="input"
                        onChange={(event) => onUpdateHint(node.clientId, hint.clientId, "longitude", event.target.value)}
                        type="number"
        ...
        <>
          <label className="field">
            ...
            <input
              className="input"
              min={1}
              onChange={(event) => onUpdateNode(node.clientId, "defaultTimeBudgetMinutes", event.target.value)}
              type="number"
              value={node.defaultTimeBudgetMinutes}
            />
            <span className="field-hint">Optional override for all descendants that do not define a leaf budget.</span>
          </label>
          ...
          <section className="node-subsection">
            <div className="mission-list-header">
              ...
              <div className="node-actions">
                <button className="ghost-button" onClick={() => onAddChild(node.clientId, "leaf")} type="button">
                  Add stage
                </button>
                <button className="ghost-button" onClick={() => onAddChild(node.clientId, "composite")} type="button">
                  Add block
                </button>
              </div>
            </div>
...
export function MissionsAdminWorkspace({ accessToken }: MissionsAdminWorkspaceProps) {
  ...
  const missionTreeStats = useMemo(() => summarizeMissionTree(draft.nodes), [draft.nodes]);
  ...
  const loadMissionDetail = useCallback(
    async (missionId: string) => {
      ...
      try {
        ...
        if (requestSequence !== detailRequestSequenceRef.current || mission.id !== missionId) {
          return;
        }
  ...
  const updateDraftField = useCallback((field: keyof MissionDraft, value: string) => {
    setDraft((current) => ({
      ...current,
      [field]: value
    }));
  ...
  const updateNodeField = useCallback(
    (nodeClientId: string, field: keyof MissionNodeDraft, value: string | boolean) => {
      setDraft((current) => ({
        ...current,
  ...
  const updateHintField = useCallback(
    (nodeClientId: string, hintClientId: string, field: keyof MissionHintDraft, value: string | boolean) => {
      setDraft((current) => ({
        ...current,
        nodes: updateNodeInTree(current.nodes, nodeClientId, (node) => ({
          ...node,
  ...
  const addRootNode = useCallback((kind: "leaf" | "composite") => {
    setDraft((current) => ({
      ...current,
      nodes: [...current.nodes, createEmptyNodeDraft(kind, getNextOrder(current.nodes))]
    }));
  }, []);

  const addChildNode = useCallback((nodeClientId: string, kind: "leaf" | "composite") => {
    setDraft((current) => ({
      ...current,
      nodes: updateNodeInTree(current.nodes, nodeClientId, (node) => ({
        ...node,
        children: [...node.children, createEmptyNodeDraft(kind, getNextOrder(node.children))]
      }))
    }));
  }, []);
  ...
  const removeHint = useCallback((nodeClientId: string, hintClientId: string) => {
    setDraft((current) => ({
      ...current,
      nodes: updateNodeInTree(current.nodes, nodeClientId, (node) => ({
        ...node,
  ...
  const makeComposite = useCallback((nodeClientId: string) => {
    setDraft((current) => ({
      ...
      nodes: updateNodeInTree(current.nodes, nodeClientId, (node) => ({
        ...node,
        children: node.children.length > 0 ? node.children : [createEmptyNodeDraft("leaf", 1)],
        hints: [],
        timeBudgetMinutes: "",
        expectedQrHash: "",
        triviaValidAnswer: "",
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const requestUrl = editorMode === "create" ? missionsUrl : `${missionsUrl}/${selectedMissionId}`;
    ...
    <section className="panel stack-gap">
      <div className="section-heading">
        ...
        <p className="section-copy">
          Reading this as: administrator control surface for mission authors, with a calm operational language,
          leaning toward the existing warm shell plus denser nested editing blocks.
        </p>
      </div>
          ...
          <form className="auth-form" onSubmit={handleSubmit}>
            <div className="tree-stat-grid">
              ...
              <article className="signal-card">
                ...
                <p className="metric-value">{missionTreeStats.compositeBlocks}</p>
              </article>
              <article className="signal-card">
                <strong>Hints</strong>
            ...
            <div className="form-grid-two">
              ...
              <label className="field">
                ...
                <input
                  ...
                  max={1440}
                  min={1}
                  onChange={(event) => updateDraftField("maximumDurationMinutes", event.target.value)}
                  required
                  type="number"
            ...
            <label className="field">
              ...
              <select
                className="input"
                onChange={(event) => updateDraftField("gameType", event.target.value)}
              ...
              <span className="field-hint">
                Mission stages can mix game types now. Keep this catalog label aligned with how the Mission should be
                listed in current backend summaries.
              </span>
            </label>
            ...
            <section className="node-subsection">
              <div className="mission-list-header">
                ...
                <div className="node-actions">
                  <button className="ghost-button" onClick={() => addRootNode("leaf")} type="button">
                    Add root stage
                  </button>
                  <button className="ghost-button" onClick={() => addRootNode("composite")} type="button">
                    Add root block
                  </button>
                </div>
              </div>
              ...
              {draft.nodes.length === 0 ? (
                <div className="tree-empty-state">
                  ...
                  <p>Start with a playable stage or a composite block that nests a deeper flow.</p>
                </div>
              ) : (
                <div className="mission-node-list">
